...
export default function StatCard({ label, value, subLabel, icon: Icon, variant = "navy" }) {
    ...
    <div className={`relative overflow-hidden rounded-2xl p-5 shadow-sm ${v.card}`}>
      ...
      <div className="mt-4 flex items-end justify-between gap-2">
        <div>
          ...
          <p className={`mt-1 text-lg font-medium ${v.label}`}>{label}</p>
        </div>
      ...
      {subLabel && (
        <p className={`mt-1 text-sm ${v.subLabel}`}>{subLabel}</p>
      )}
    </div>
  );
}