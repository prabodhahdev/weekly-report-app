        ...
        {blockers.map((blocker, index) => {
          const text =
            typeof blocker === "string"
              ? blocker
              : blocker.text || blocker.description || blocker.blocker;

          const isKey =
            typeof blocker === "object" &&
            (blocker.isKey || blocker.isKeyIssue);

          return (
            <div
              key={blocker._id || blocker.id || index}
              className={`flex items-start gap-3 rounded-lg p-4 ${
                isKey ? "border border-red-200 bg-red-50" : "bg-[#f2f2f2]"
              }`}
            >
              <AlertTriangle
                size={17}
                className={`mt-0.5 shrink-0 ${
                  isKey ? "text-red-500" : "text-[#9ca3af]"
                }`}
              />

              <div className="flex-1">
                <p className="text-sm leading-6 text-[#1b3040]">{text || "-"}</p>
              </div>
